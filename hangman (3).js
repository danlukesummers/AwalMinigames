...
function hgSetSource(src){
  if(src === 'ai' && hgState.plan !== 'paid'){
    document.getElementById('hg-start-error').textContent = '✨ AI topic generator is a paid feature. Switch the demo account toggle to "Paid" to try it.';
...
function hgShowHint(customHintText){
  ...
  const hint = customHintText || HG_HINTS[currentWord] || ('Starts with "' + currentWord.charAt(0).toUpperCase() + '" and ends with "' + currentWord.charAt(currentWord.length - 1).toUpperCase() + '".');
  if(hintTextEl){
...
function hgRevealWord(word){
  const wordDisplay = document.getElementById('hg-word-display');
  if(!wordDisplay || !word) return;

  // Always replace the hidden letters with the complete word when the round ends.
  // This is deliberately separate from hgRenderWord() so the failed word cannot
  // be re-hidden by any later keyboard/progress update.
  wordDisplay.innerHTML = word.split('').map(ch =>
    '<div class="hg-letter-box hg-letter-revealed">' + ch + '</div>'
  ).join('');
  wordDisplay.classList.add('hg-word-revealed');
}
...
function hgWordEnd(won){
  ...
  // Reveal the complete word for BOTH success and failure.
  // The revealed word stays visible until the player starts the next word.
  hgRevealWord(word);
...
async function hgBeginRound(){
  ...
  if(hintBtn) hintBtn.addEventListener('click',hgSendBroadcastHint);
}
...
function hgRenderDashboardCard(student,word){
  ...
  document.getElementById(student.id + '-guesses').textContent = student.guessed.length ? 'Guesses: ' + student.guessed.join(', ').toUpperCase() : '';
...
function hgFinishStudent(student,won,word){
  ...
  if(won){
    ...
    document.getElementById(student.id + '-word').innerHTML = word.split('').map(ch => '<div class="hg-dash-letter">' + ch + '</div>').join('');
